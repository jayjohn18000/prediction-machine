...
| Location | Why |
|----------|-----|
| **https://www.pmxt.dev/docs** — “Quick Start” and “Methods” (fetchMarkets, fetchEvents, fetchOrderBook, fetchOHLCV) | Shows the single, consistent API surface and the “initialize exchange → same calls for Poly/Kalshi/Limitless” flow. **Screenshot recommended.** |
| **https://www.pmxt.dev/docs** — “Data Models” (UnifiedMarket, UnifiedEvent, MarketOutcome) | Makes the Event → Market → Outcome hierarchy and shared fields (outcomeId, yes/no, etc.) visible at a glance. **Screenshot recommended.** |
| **https://www.pmxt.dev/docs** — “Complete Trading Workflow” | One end-to-end flow (balance → search → createOrder → fetchOrder → cancelOrder → positions) in one block; good for “one integration layer” storytelling. **Screenshot recommended.** |
| **https://github.com/pmxt-dev/pmxt** — README “Quickstart” (Event/Market/Outcome bullets + code) | Reinforces the hierarchy and the “search events → pick market → use outcome” pattern. **Screenshot recommended.** |
| **https://github.com/pmxt-dev/pmxt** — ARCHITECTURE.md “The Sidecar Pattern” diagram | The ASCII diagram (Python/TS SDKs → HTTP → Node server → Exchange APIs) is the clearest view of the sidecar. **Screenshot recommended.** |
| **https://github.com/pmxt-dev/pmxt** — `core/COMPLIANCE.md` feature matrix | Table of which exchange supports which function (fetchMarkets, fetchOHLCV, createOrder, etc.) and “Partial / Skipped” notes. **Screenshot recommended.** |
| **https://pmxt.dev/** — Landing hero (“The ccxt for prediction markets” + code snippet) | Product positioning and the “universal client” idea. Optional. |

---

*Summary derived from pmxt monorepo (JS + Python), https://www.pmxt.dev/docs and https://pmxt.dev/, and repo docs (ARCHITECTURE.md, COMPLIANCE.md, readme). No code copied; concepts only.*
