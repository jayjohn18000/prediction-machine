 ...
 *
 * `paper` uses production REST + WS (live book) but defers order placement to the DB path (Stream D).
 */
import "dotenv/config";

/** @typedef {'demo' | 'prod' | 'paper'} KalshiRunMode */

/**
 * Resolve the active Kalshi run mode. Default: 'demo' unless MM_RUN_MODE=prod|paper.
 *
...
export function deriveWsUrlFromRest(restBase, runMode) {
  try {
    ...
    const expectedHost =
      runMode === "prod" || runMode === "paper" ? /(^|\.)kalshi\.com$/i : /demo-api\.kalshi\.co$/i;
    if (!expectedHost.test(u.hostname)) {
      // Mismatch between explicit REST host and run mode — return the mode default.
      return runMode === "prod" || runMode === "paper" ? PROD_WS_DEFAULT : DEMO_WS_DEFAULT;
    }
    u.protocol = "wss:";
...
export function guardKalshiTradingBase(baseUrl, runMode) {
  ...
  try {
    ...
    const prodLike = runMode === "prod" || runMode === "paper";
    const expected = prodLike ? /(^|\.)kalshi\.com$/i : /demo-api\.kalshi\.co$/i;
...
/** `MM_RUN_MODE=paper`: prod endpoints, no live Kalshi order POST (orchestrator uses DB paper path when enabled). */
export function isPaperModeEnabledFromEnv() {
  const v = process.env.MM_PAPER_MODE_ENABLED?.trim().toLowerCase();
