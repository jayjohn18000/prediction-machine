...
export async function fetchPortfolioDailyNetPnLCentsUtc(client) {
  const r = await client.query(`
    SELECT COALESCE(SUM(s.net_pnl_cents), 0)::numeric AS n
    FROM (
      SELECT DISTINCT ON (market_id) market_id, net_pnl_cents
...
export async function checkPreTrade(ctx) {
  ...
  const toxThresh =
    toxicityThresholdFromEnv() ?? Number(cfg.toxicity_threshold ?? 500);
  const tox = cfg.last_toxicity_score != null ? Number(cfg.last_toxicity_score) : null;
  if (tox != null && Number.isFinite(tox) && tox >= toxThresh) {
...
/**
 * Append-only operational alert (audit row only — does not flip kill_switch_active).
 *
 * @param {import('pg').Client | import('pg').PoolClient} client
 * @param {{ marketId?: number|string|null, reason: string, details?: object }} p
 */
export async function appendMmOperationalEvent(client, p) {
  await client.query(
    `
    INSERT INTO pmci.mm_kill_switch_events (market_id, reason, details)
    VALUES ($1::bigint, $2::text, $3::jsonb)
    `,
    [p.marketId ?? null, p.reason, JSON.stringify(p.details ?? {})],
  );
}
