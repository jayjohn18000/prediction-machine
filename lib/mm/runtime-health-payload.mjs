...
export function buildMmHealthMmResponse(p) {
  ...
  const skipPlacement = Array.isArray(h.mmSkippedPlacementTickers) ? h.mmSkippedPlacementTickers : [];
  const placementSkipOk = skipPlacement.length === 0;

  const watchdogAlerts = Array.isArray(h.mmWatchdogActiveAlerts) ? h.mmWatchdogActiveAlerts : [];
  /** Reasons that appear in mmWatchdogActiveAlerts for visibility but must not block readiness. */
  const readyNonBlockingWatchdogReasons = new Set(["worst_trade_alarm"]);
  const watchdogBlocking = watchdogAlerts.filter(
    (a) => !readyNonBlockingWatchdogReasons.has(String(/** @type {any} */ (a).reason)),
  );
  const watchdogOk = watchdogBlocking.length === 0;

  const ready =
    baseOk && depthConnectedOk && depthStaleOk && loopFresh && placementSkipOk && watchdogOk;
