...
import { upsertPositionFromMmFill } from "./position-store.mjs";
 ...
 * @param {{ phase: string, skipped: boolean }} reconciliation
 */
...
export async function ingestFillsForTicker(client, trader, ticker, marketPk) {
  ...
  for (const f of j.fills ?? []) {
    ...
    try {
      ...
      // Preserve fractional fills; Kalshi DEMO frequently partials (e.g. count_fp=0.02).
      // The prior `Math.max(1, Math.round(fp))` inflated 0.02-contract fills to 1.0.
      if (!Number.isFinite(fp) || fp <= 0) continue;
      // Trust the parent order's mm_side; Kalshi normalizes fill `side`/`action` to the
      // contract side (e.g. a yes_sell ack comes back as side="no", action="sell"), so
      // mapKalshiFillToMmSide(f) double-flips and breaks position rollup. The parent row
      // carries the side we intended at placement, which is what the rollup needs.
...
async function placeLimitRow(p) {
  ...
  try {
    ...
    await updateMmOrderStatus(client, row.id, "errored");
    throw e;
  }
