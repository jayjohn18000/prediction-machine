...
/**
 * Snapshot before applying a new fill (advisory alarms, not authoritative vs recompute).
 *
 * @param {import('pg').Client | import('pg').PoolClient} client
 * @param {number|string} marketId
 * @returns {Promise<{ net_contracts: number, avg_cost_cents: number|null }>}
 */
export async function fetchMmPositionSnapshot(client, marketId) {
  const r = await client.query(
    `SELECT net_contracts, avg_cost_cents FROM pmci.mm_positions WHERE market_id = $1::bigint`,
    [marketId],
  );
  const row = r.rows[0];
  if (!row) return { net_contracts: 0, avg_cost_cents: null };
  return {
    net_contracts: row.net_contracts != null ? Number(row.net_contracts) : 0,
    avg_cost_cents:
      row.avg_cost_cents != null && row.avg_cost_cents !== "" ? Number(row.avg_cost_cents) : null,
  };
}
  ...
  try {
    ...
    await client.query(
      ...
      INSERT INTO pmci.mm_positions (
        ...
        last_updated
      ) VALUES ($1::bigint, $2::numeric, $3::numeric, $4::numeric, $5::numeric, COALESCE($6::timestamptz, now()))
