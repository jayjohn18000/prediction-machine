...
/**
 * Postgres `best_*_cents_at_place` columns are integer; REST dollar→cent math
 * can yield floats like 57.99999999999999 (`yes_bid_dollars * 100`).
 *
 * @param {unknown} v
 * @returns {number|null}
 */
export function roundNullableMmBookCents(v) {
...
export async function insertMmOrderPending(client, row) {
  let depthJson = null;
  if (row.book_depth_at_place_jsonb != null) {
    depthJson =
      typeof row.book_depth_at_place_jsonb === "string"
        ? row.book_depth_at_place_jsonb
        : JSON.stringify(row.book_depth_at_place_jsonb);
  }
  const basePayload =
    row.payload != null && typeof row.payload === "object" && !Array.isArray(row.payload)
      ? { ...row.payload }
      : {};
  if (row.mode === "paper") basePayload.mm_mode = "paper";
  ...
  const params = [
    ...
    payloadJson,
    roundNullableMmBookCents(row.best_bid_cents_at_place),
    roundNullableMmBookCents(row.best_ask_cents_at_place),
    depthJson,
  ];

  const modeNorm = row.mode != null && String(row.mode).trim() !== "" ? String(row.mode).trim() : null;
  if (modeNorm && modeNorm !== "live") {
    try {
      const r = await client.query(
        `
        INSERT INTO pmci.mm_orders (
          ...
          best_bid_cents_at_place, best_ask_cents_at_place, book_depth_at_place_jsonb,
          mode
        ) VALUES (
...
export async function insertFill(client, p) {
  const fvRes = await client.query(
    `SELECT fair_value_at_place, placed_at FROM pmci.mm_orders WHERE id = $1`,
    [p.order_pk],
  );
  const fvPlace = fvRes.rows[0]?.fair_value_at_place;
  const placedAt = fvRes.rows[0]?.placed_at;
  ...
  const obsMs = new Date(p.observed_at).getTime();
  const placedMs = placedAt ? new Date(placedAt).getTime() : NaN;
  const quoteAgeMs =
    p.quote_age_ms_at_fill != null && Number.isFinite(Number(p.quote_age_ms_at_fill))
      ? Math.round(Number(p.quote_age_ms_at_fill))
      : Number.isFinite(obsMs) && Number.isFinite(placedMs)
        ? Math.max(0, Math.round(obsMs - placedMs))
        : null;

  const nf = p.kalshi_net_fee_cents;
  const tf = p.kalshi_trade_fee_cents;
  const rf = p.kalshi_rounding_fee_cents;
  const rb = p.kalshi_rebate_cents;

  /** @type {boolean|null} Rebate >0 => maker; explicit trade fee >0 => taker; else NULL (pre–fee-tracking fills). */
  let wasMaker = null;
  if (rb != null && Number(rb) > 0) wasMaker = true;
  else if (tf != null && Number(tf) > 0) wasMaker = false;

  const entryPrice = Number(p.price_cents) / 100.0;
  ...
  if (p.kalshi_fill_id) {
    const ins = `
      INSERT INTO pmci.mm_fills (
        ...
        kalshi_net_fee_cents, kalshi_trade_fee_cents, kalshi_rounding_fee_cents, kalshi_rebate_cents,
        quote_age_ms_at_fill,
        entry_price, was_maker
      )
