...
/** Extra cents allowed beyond `half` when enforcing distance-to-fair (default 0). */
export function quoteSlippageBufferCents() {
  const raw =
    typeof globalThis.process !== "undefined" ? globalThis.process.env?.MM_QUOTE_SLIPPAGE_BUFFER_CENTS : undefined;
...
/**
 * Clamp YES-cents quote onto [fair − half − buffer, fair + half + buffer].
 *
 * @param {number|null} px
 * @param {number} fair 1–99
 * @param {number} halfSpread widened half-spread for this tick
 * @param {number} [buffer] optional; defaults via quoteSlippageBufferCents()
 */
export function clampQuotePxToFairBand(px, fair, halfSpread, buffer) {
  if (px == null) return null;
  const buf = buffer !== undefined ? Math.max(0, Math.floor(Number(buffer) || 0)) : quoteSlippageBufferCents();
  const h = Math.max(0, Number(halfSpread) || 0);
  const dev = h + buf;
  return clampInt(px, fair - dev, fair + dev);
}
 ...
 * @param {{ bestBidCents?: number|null, bestAskCents?: number|null }} [p.topOfBook] Kalshi top-of-book (YES cents); when present, quotes clamp to avoid crossing the live book for post_only.
 * @param {{
...
export function decideQuote(p) {
  ...
  let bidSkippedReason;
  /** @type {string | undefined} */
  let askSkippedReason;
  const tob = p.topOfBook;
  if (tob?.bestAskCents != null && Number.isFinite(Number(tob.bestAskCents))) {
    const askTop = Math.round(Number(tob.bestAskCents));
    const maxBid = askTop - 1;
    bidPx = Math.min(bidPx, maxBid);
    if (bidPx < 1) {
      bidSkippedReason = "would_cross";
      console.log(`mm.quote_skip reason=would_cross side=bid bestAsk=${askTop} wouldBe=${bidPx}`);
      bidPx = null;
    }
  }
  if (tob?.bestBidCents != null && Number.isFinite(Number(tob.bestBidCents))) {
    const bidTop = Math.round(Number(tob.bestBidCents));
    const minAsk = bidTop + 1;
    askPx = Math.max(askPx, minAsk);
    if (askPx > 99) {
      askSkippedReason = "would_cross";
      console.log(`mm.quote_skip reason=would_cross side=ask bestBid=${bidTop} wouldBe=${askPx}`);
      askPx = null;
    }
  }

  const slipBuf = quoteSlippageBufferCents();
  if (bidPx != null) {
    bidPx = clampQuotePxToFairBand(bidPx, fair, half, slipBuf);
    bidPx = clampInt(bidPx, 1, 98);
  }
  if (askPx != null) {
    askPx = clampQuotePxToFairBand(askPx, fair, half, slipBuf);
    askPx = clampInt(askPx, 3, 99);
  }
  if (bidPx != null && askPx != null && askPx <= bidPx) {
    bidPx = clampInt(fair - half, 1, 98);
    askPx = clampInt(fair + half, Math.min(99, bidPx + 2), 99);
    if (bidPx != null) {
      bidPx = clampQuotePxToFairBand(bidPx, fair, half, slipBuf);
    ...
    if (bidPx != null && askPx != null && askPx <= bidPx) {
      askPx = clampInt(Math.min(99, bidPx + 2), 3, 99);
    }
  }
  ...
  return {
    ...
    ...(bidSkippedReason != null ? { bidSkippedReason } : {}),
    ...(askSkippedReason != null ? { askSkippedReason } : {}),
  };
}
