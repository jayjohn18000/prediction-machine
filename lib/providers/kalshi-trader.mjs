...
export class KalshiTrader {
  ...
  buildCreateOrderBody(p) {
    ...
    const cnt = Math.max(1, Math.round(Number(sizeContracts)));
    const priceDollars = (Math.round(pc) / 100).toFixed(4);
    const countFp = cnt.toFixed(2);
    const deciCent = String(priceLevelStructure ?? "") === "deci_cent";
    ...
    if (mmSide === "yes_buy" || mmSide === "yes_sell") {
      ...
      const yp = Math.round(pc);
      if (deciCent) body.yes_price_dollars = priceDollars;
      else body.yes_price = yp;
    } else {
      body.side = "no";
      body.action = mmSide === "no_buy" ? "buy" : "sell";
      const np = Math.round(pc);
      if (deciCent) body.no_price_dollars = priceDollars;
      else body.no_price = np;
    }
    return body;
  }

  /**
   * @param {{ ticker: string, mmSide: string, priceCents: number, sizeContracts: number, clientOrderId: string, postOnly?: boolean, priceLevelStructure?: string | null }} p
   */
