...
export class PmciClient {
  ...
  /**
   * Top cross-provider divergences globally or filtered by event/category. Includes soft staleness via `data_lag_seconds` (no 503 when observer is behind).
   * @param {{ event_id?: string, category?: string, limit?: number }} params
   * @returns {Promise<{ data_lag_seconds: number | null, families: Array<import("./pmci-client.types.d.ts").TopDivergenceResult> }>}
   */
  getTopDivergences({ event_id, category, limit } = {}) {
    return this._request("GET", "/v1/signals/top-divergences", { query: { event_id, category, limit } });
  }
