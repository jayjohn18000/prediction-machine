 ...
 *   PMCI_ECONOMICS_KALSHI_SERIES_TICKERS — comma list (default includes Fed + GDP/CPI/NFP macro series)
 *   PMCI_ECONOMICS_KALSHI_MAX_EVENTS — cap per run (default 40)
 *   PMCI_ECONOMICS_POLY_TAG_ID — optional Gamma tag_id for macro events
 *   PMCI_ECONOMICS_POLY_SLUG_KEYWORDS — comma keywords for public-search (macro: fed, gdp, nfp, …)
...
async function ingestKalshiEconomics(pmciClient, providerId, opts) {
  const seriesTickers = splitCsv(
    process.env.PMCI_ECONOMICS_KALSHI_SERIES_TICKERS ||
      "KXFEDDECISION,KXRATECUTCOUNT,KXGDP,KXCPI,KXNFP",
  );
...
async function ingestPolymarketEconomics(pmciClient, providerId, opts) {
  ...
  const slugKeywords = splitCsv(
    process.env.PMCI_ECONOMICS_POLY_SLUG_KEYWORDS ||
      "fed,fomc,cpi,rate cut,gdp,nfp,employment,unemployment,jobs report,recession",
  );
