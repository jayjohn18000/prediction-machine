...
/** Aligns with observer frontier policy: prefer rows that participate in an active link. */
const SQL_STALE_MARKETS_PRIORITIZE_LINKED = `
  SELECT pm.id, pm.provider_market_ref, pm.event_ref, p.code AS provider_code
  FROM pmci.provider_markets pm
  JOIN pmci.providers p ON p.id = pm.provider_id
  WHERE (pm.status IS NULL OR pm.status IN ('open', 'active'))
...
const PRIORITIZE_LINKED = process.env.PMCI_SWEEP_PRIORITIZE_LINKED === '1';
...
export async function runPmciSweep({ pmciClient, pmciIds, observedAt }) {
  ...
  const sql = PRIORITIZE_LINKED ? SQL_STALE_MARKETS_PRIORITIZE_LINKED : SQL_STALE_MARKETS;
  const res = await pmciClient.query(sql, [BATCH_LIMIT]);
  const rows = res.rows ?? [];
