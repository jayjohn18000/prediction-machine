...
/** Delay between outbound `subscribe` frames (Kalshi DEMO WS mishandles burst subscribes; Track I 2026-05-01). */
export const DEFAULT_SUBSCRIBE_SPACING_MS = 120;
...
/**
 * Normalize a Kalshi level tuple to `[priceCents:number, qty:number]`.
 *
 * Kalshi WS now sends `yes_dollars_fp` / `no_dollars_fp` as `[priceDollarsString, qtyString]`
 * (e.g. `["0.2600","11.00"]`). The legacy `yes` / `no` `[priceCents, qty]` form is also accepted
 * for backward compatibility with replay fixtures and unit tests.
 *
 * @param {[unknown, unknown]} levelTuple
 * @returns {[number, number] | null} null if the tuple cannot be parsed
 */
export function normalizeLevelTuple(levelTuple) {
  if (!Array.isArray(levelTuple) || levelTuple.length < 2) return null;
  const [rawPrice, rawQty] = levelTuple;
  const priceNum = typeof rawPrice === "string" ? Number.parseFloat(rawPrice) : Number(rawPrice);
  const qtyNum = typeof rawQty === "string" ? Number.parseFloat(rawQty) : Number(rawQty);
  if (!Number.isFinite(priceNum) || !Number.isFinite(qtyNum)) return null;
  // Heuristic: dollar-decimals (≤ 1.0 with non-integer cents) come from `*_dollars_fp`;
  // legacy integer cents come from the older `yes`/`no` form.
  const looksLikeDollars =
    typeof rawPrice === "string" || (priceNum > 0 && priceNum <= 1 && !Number.isInteger(priceNum));
  const priceCents = looksLikeDollars ? Math.round(priceNum * 100) : Math.round(priceNum);
  if (priceCents < 0 || priceCents > 100) return null;
  return [priceCents, qtyNum];
}

function pickSidePayload(payload, side) {
  // Prefer the new `*_dollars_fp` field; fall back to the legacy field name.
  const fpField = side === "yes" ? "yes_dollars_fp" : "no_dollars_fp";
  const legacyField = side; // "yes" | "no"
  if (Array.isArray(payload?.[fpField])) return payload[fpField];
  if (Array.isArray(payload?.[legacyField])) return payload[legacyField];
  return null;
}
...
export function applySnapshot(book, payload) {
  book.yes.clear();
  book.no.clear();
  const yesLevels = pickSidePayload(payload, "yes") ?? [];
  const noLevels = pickSidePayload(payload, "no") ?? [];
  for (const tuple of yesLevels) {
    const norm = normalizeLevelTuple(tuple);
    if (norm && norm[1] > 0) book.yes.set(norm[0], norm[1]);
  }
  for (const tuple of noLevels) {
    const norm = normalizeLevelTuple(tuple);
    if (norm && norm[1] > 0) book.no.set(norm[0], norm[1]);
  }
 ...
 * Leaves a side untouched when the corresponding `*_dollars_fp` (or legacy) field is absent.
 */
export function applyDelta(book, payload) {
  for (const side of /** @type {const} */ (["yes", "no"])) {
    const levels = pickSidePayload(payload, side);
    if (!Array.isArray(levels)) continue;
    const m = book[side];
    for (const tuple of levels) {
      const norm = normalizeLevelTuple(tuple);
      if (!norm) continue;
      const [priceCents, qty] = norm;
      if (qty === 0 || qty == null) {
        m.delete(priceCents);
      } else {
        m.set(priceCents, qty);
      }
    }
  }
  book.lastUpdateMs = Date.now();
...
/**
 * Wrap a depth row writer: log level counts, skip DB write when `yes_levels` is empty (WARN).
 * Exported for tests and to keep persistence rules explicit.
 *
 * @param {function(object): Promise<void>} writeRow
 * @param {object} [logger]
 */
export function wrapDepthRowWriter(writeRow, logger = console) {
  return async function guardedWriteDepthRow(row) {
    const yesN = row?.yes_levels?.length ?? 0;
    const noN = row?.no_levels?.length ?? 0;
    logger.info?.("depth.persist.level_counts", {
      provider_market_id: row?.provider_market_id,
      yes_levels: yesN,
      no_levels: noN,
    });
    if (yesN === 0) {
      logger.warn?.("depth.persist.skip_empty_yes", {
        provider_market_id: row?.provider_market_id,
 ...
 * Returns { stop, books, getHealthSnapshot } where books is the in-memory book map (for observability / tests).
 *
 * @param {object} opts
 ...
 * @param {number} [opts.subscribeSpacingMs=DEFAULT_SUBSCRIBE_SPACING_MS] - pause between subscribe sends; 0 disables (not recommended for multi-ticker DEMO WS)
...
export async function startDepthIngestion({
  ...
  const writeRow = wrapDepthRowWriter(innerWrite, logger);
  ...
  const STALE_QUOTE_SEC = 60;

  // Fresh signing timestamp per connection — Kalshi rejects stale KALSHI-ACCESS-TIMESTAMP (401).
  ...
  /** WebSocket-protocol PING every 25s (Track J — DEMO subs go idle without keepalive). */
  ...
  const startHeartbeat = () => {
    ...
    heartbeatTimer = setInterval(() => {
      ...
      try {
        ...
        logger.warn?.("depth.heartbeat.ping_failed", { err: String(err) });
      }
    }, 25_000);
  ...
  /** >50% tickers with no quotes older than `STALE_QUOTE_SEC * 2` → backoff reconnect (Track J Layer 2). */
  let stopStalenessWatchdogCleanup = null;
  const startStalenessWatchdog = () => {
    const watchdogTimer = setInterval(() => {
      if (userStopped) return;
      if (!ws || ws.readyState !== WebSocketCtor.OPEN) return;
      let staleCount = 0;
      for (const t of marketTickers) {
        const sec = secondsSinceLastUpdate(t, books);
        if (!Number.isFinite(sec) || sec > STALE_QUOTE_SEC * 2) staleCount += 1;
      }
      if (staleCount > marketTickers.length / 2) {
        logger.warn?.("depth.watchdog.force_reconnect", {
          stale_count: staleCount,
          total: marketTickers.length,
        });
  ...
  const subscribeAll = async (socket) => {
    let subscribeId = 1;
    for (let i = 0; i < marketTickers.length; i++) {
      const ticker = marketTickers[i];
      socket.send(
        JSON.stringify({
          id: subscribeId++,
          cmd: "subscribe",
      ...
      const gap = subscribeSpacingMs > 0 && i + 1 < marketTickers.length ? subscribeSpacingMs : 0;
      if (gap > 0) await sleepSubscribeGap(gap);
    }
  };
  ...
  const attachSocket = () => {
    ...
    stopHeartbeat();
    if (stopStalenessWatchdogCleanup) {
      stopStalenessWatchdogCleanup();
      stopStalenessWatchdogCleanup = null;
    }
    if (ws) {
      try {
    ...
    socket.on("pong", () => {
      // Layer 1: diagnostic hook; Layer 2 may reset watchdog state here.
    });
    ...
    socket.on("open", () => {
      ...
      void subscribeAll(socket).then(() => {
        startHeartbeat();
        if (stopStalenessWatchdogCleanup) {
          stopStalenessWatchdogCleanup();
          stopStalenessWatchdogCleanup = null;
        }
        stopStalenessWatchdogCleanup = startStalenessWatchdog();
      });
      if (!stopDownsampler) {
  ...
  const getHealthSnapshot = () => {
    const configured = marketTickers.length;
    const connected = ws != null && ws.readyState === WebSocketCtor.OPEN;
    const depthSubscribedConnected = connected ? configured : 0;
    /** @type {Record<string, number|null>} */
    const depthLastUpdateSecondsAgo = {};
    /** @type {string[]} */
    const depthTickersStale = [];
    for (const t of marketTickers) {
      const raw = secondsSinceLastUpdate(t, books);
      const sec = Number.isFinite(raw) ? raw : null;
      depthLastUpdateSecondsAgo[t] = sec;
      if (!connected || raw === Number.POSITIVE_INFINITY || (sec != null && sec > STALE_QUOTE_SEC)) {
        depthTickersStale.push(t);
      }
    }
    return {
      depthSubscribedConfigured: configured,
      depthSubscribedConnected,
