...
/** D7 — Derive election phase from ticker/title. */
...
/** D7 — Derive subject type from ticker structure and title. */
          ...
          for (const m of markets) {
            ...
            const result = await ingestProviderMarket(
              pmciClient,
              {
                ...
                electionPhase: inferElectionPhase(seriesTicker, marketTitle),
                subjectType: inferSubjectType(seriesTicker, marketTitle),
                priceYes,
          ...
          for (const { k, outcomeName } of outcomesToIngest) {
            ...
            const result = await ingestProviderMarket(
              pmciClient,
              {
                ...
                category: "politics",
                status: baseStatus,
                electionPhase: inferElectionPhase(slug, title),
                subjectType: inferSubjectType(slug, title),
                metadata: {
                  source: "pmci-ingest-politics-universe",
                  mode: "universe",
                  provider: "polymarket",
