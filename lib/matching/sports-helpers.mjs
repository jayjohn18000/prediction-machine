...
/**
 * Lowercase + alphanumeric-only + strip common generic affixes that appear
 * inconsistently across Kalshi/Polymarket team naming.
 *
 * Strippable affixes intentionally include ONLY generic tokens that do NOT
 * distinguish two different clubs. Club-distinguishing tokens (like `united`,
 * `city`, `athletic`, `rovers`, `wanderers`) are preserved — stripping them
 * would collapse e.g. "Manchester United" and "Manchester City".
 *
 * v2-h2h (Phase Linker H2H Expansion, 2026-04-24): added `afc` and the
 * multiword `saudi club` suffix (which appears on nearly every Polymarket
 * Saudi Pro League team but never on Kalshi's versions).
 */
export function normalizeTeamName(name) {
  return String(name || '')
    .toLowerCase()
    // Strip the generic multi-word "saudi club" suffix first (before punctuation is stripped).
    .replace(/\bsaudi\s+club\b/g, ' ')
    // Drop punctuation
    .replace(/[^a-z0-9\s]/g, ' ')
    // Strip generic single-token affixes (club type / non-distinguishing markers only).
    .replace(/\b(fc|cf|sc|afc|club|team)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
const BUCKET_PATTERNS = [
  // Draw / tie phrasing (Phase G Phase 3): align with classifyPhaseGSportsMarketType moneyline bucket
