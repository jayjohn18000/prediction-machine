...
/**
 * v2-h2h: feature flag for Phase Linker H2H Expansion's "teams-match bypass".
 * When enabled AND both sides have populated home/away teams AND both sides
 * share category='sports' AND at least one side has sport='unknown', the
 * sport-equality gate is bypassed and a fuzzy team-name match produces a
 * *reduced* attachment score (0.6) carrying `reasons.bypass_reason =
 * 'teams_match_unknown_sport'`. A3 equivalence re-audit is the hard gate.
 *
 * Default OFF in production. Enable during a targeted auto-link pass.
 */
export function isLinkerH2HTeamsBypassEnabled() {
  const v = String(process.env.LINKER_H2H_TEAMS_BYPASS || "").trim().toLowerCase();
  return v === "true" || v === "1" || v === "yes" || v === "on";
}

/**
 * Detailed sports attachment score + reasons payload. Sport-unknown bypass
 * gated by env LINKER_H2H_TEAMS_BYPASS (see isLinkerH2HTeamsBypassEnabled).
 *
 * @param {{
 ...
 *   sport?: string
 * }} market
 * @returns {{ score: number, reasons: Record<string, unknown> }}
 */
export function scoreSportsAttachmentDetailed(event, market) {
  ...
  if (!mh || !ma || !eh || !ea) {
    return { score: 0, reasons: { reason: "missing_team", eh, ea, mh, ma } };
  }
  ...
  const fuzzyTeams = !exactTeams && (directFuzzy || swapFuzzy);

  // v2-h2h teams-match bypass: triggers only under the feature flag, when both
  // sides share category='sports', both have all teams populated, and at least
  // one side has sport='unknown'. Returns a REDUCED score (0.6) so A3 re-audit
  // has a tighter hard gate than ordinary sport-equality matches.
  const eventCat = String(event?.category || "").toLowerCase();
  const marketCat = String(market?.category || "").toLowerCase();
  const eventSport = String(event?.sport || "").toLowerCase();
  const marketSport = String(market?.sport || "").toLowerCase();
  const eitherSideUnknown =
    !eventSport || eventSport === "unknown" ||
    !marketSport || marketSport === "unknown";
  const bypassEligible =
    isLinkerH2HTeamsBypassEnabled() &&
    eventCat === "sports" && marketCat === "sports" &&
    eitherSideUnknown && Boolean(mh) && Boolean(ma) && Boolean(eh) && Boolean(ea);

  if (!exactTeams && !fuzzyTeams) {
    return { score: 0.08, reasons: { reason: "teams_no_match" } };
  }

  const ed = event?.event_date;
  const md = marketAnchorDate(market);
  if (!ed || !md) {
    return { score: 0.12, reasons: { reason: "missing_date" } };
  }

  const delta = sportsDateDeltaDays(md, ed);
  if (delta == null) {
    return { score: 0.12, reasons: { reason: "unparsable_date" } };
  }
  if (delta > 1) {
    return { score: 0.25, reasons: { reason: "date_delta_gt_1", delta } };
  }

  if (exactTeams) {
    const score = delta === 0 ? 1.0 : 0.92;
    return { score, reasons: { reason: "exact_teams", delta } };
  }

  // Fuzzy path: bypass carves a REDUCED score (0.6) carrying `bypass_reason`.
  // Under normal flow, keep pre-v2-h2h scores (0.9 when delta=0, 0.7 otherwise).
  if (fuzzyTeams) {
    if (bypassEligible) {
      return {
        score: 0.6,
        reasons: {
          reason: "fuzzy_teams",
          delta,
          bypass_reason: "teams_match_unknown_sport",
          event_sport: eventSport || null,
          market_sport: marketSport || null,
        },
      };
    }
    const score = delta === 0 ? 0.9 : 0.7;
    return { score, reasons: { reason: "fuzzy_teams", delta } };
  }

  return { score: 0.1, reasons: { reason: "unclassified" } };
}

/**
 * Sports: compare provider home/away to event participants; combine with game vs event date.
 * Kept as the numeric-return variant for backward-compat callers; internally delegates
 * to scoreSportsAttachmentDetailed.
 * @param {{ participants?: unknown, event_date?: string | Date, category?: string, sport?: string }} event
 * @param {{ home_team?: string, away_team?: string, game_date?: string | Date, title?: string, category?: string, sport?: string }} market
 */
function scoreSportsAttachment(event, market) {
  return scoreSportsAttachmentDetailed(event, market).score;
}
