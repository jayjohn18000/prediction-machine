/**
 * PMCI proposal engine: equivalent and proxy link proposals (Kalshi ↔ Polymarket).
 * Supports any category (politics, sports, crypto, etc.) via opts.category.
 * Caller must load env (DATABASE_URL, PMCI_MAX_PROPOSALS_*). Exports runProposalEngine({ dryRun, category }).
 *
 * Pipeline stages extracted (Step 5 decomposition):
 *   lib/matching/scoring.mjs     — pure similarity/scoring math
 *   lib/matching/entity-parse.mjs — entity normalization and per-venue parsing
 */
...
import {
  ...
  isGenericEntity,
  extractRaceTokens,
  isNumericOrGenericOutcome,
  entitySimilarityPass,
  parsePolyRef,
  parseKalshiTitle,
} from './entity-parse.mjs';
 ...
 * @returns {string}
 */
export function normalizeTopicKey(rawKey) {
  const key = String(rawKey || '').toLowerCase().trim();
  if (!key) return DEFAULT_TOPIC_KEY;

  // Cosmetic normalization: allow incoming gov_* aliases and full-state variants.
  const govCode = key.match(/^gov_([a-z]{2})_(\d{4})(?:_special)?$/i);
  if (govCode) return key.replace(/^gov_/, 'governor_');

  const govName = key.match(/^gov_([a-z_]+)_(\d{4})(?:_special)?$/i);
  if (govName) {
    const stateName = govName[1].replace(/_/g, ' ');
    const code = US_STATE_NAME_TO_CODE[stateName];
    if (code) return key.replace(/^gov_[a-z_]+_/, `governor_${code}_`);
  }

  return key;
}

export function extractTopicKey(market) {
  ...
  for (const [re, key] of TOPIC_KEY_PATTERNS) {
    if (re.test(combined)) return normalizeTopicKey(key);
  }
...
function extractTopicSignature(market) {
  ...
  if (govPartyMatch) return normalizeTopicKey(`governor_${(govPartyMatch[1] || '').toLowerCase()}_${govPartyMatch[2]}${special}`);
  const govPartyCompactMatch = ref.match(/^GOVPARTY([a-z]{2})-(\d{2})(?:-|$)/i);
  if (govPartyCompactMatch) {
    const yyyy = `20${govPartyCompactMatch[2]}`;
    return normalizeTopicKey(`governor_${(govPartyCompactMatch[1] || '').toLowerCase()}_${yyyy}${special}`);
  }
  ...
  const senateCompactMatch = ref.match(/^SENATE([a-z]{2})-(\d{2})(?:-|$)/i);
  if (senateCompactMatch) {
    const yyyy = `20${senateCompactMatch[2]}`;
    return `senate_${(senateCompactMatch[1] || '').toLowerCase()}_${yyyy}${special}`;
  }
  const houseDistrictMatch = ref.match(/^HOUSE-([a-z]{2})-(\d{1,2})-(\d{4})/i);
  if (houseDistrictMatch) {
    return `house_${(houseDistrictMatch[1] || '').toLowerCase()}${houseDistrictMatch[2]}_${houseDistrictMatch[3]}${special}`;
  }
  ...
  if (/\bhouse\b/.test(combined) && /\b(tx|texas)\s*[- ]?33(?:st|nd|rd|th)?\b/.test(combined)) {
    return `house_tx33_${year || '2026'}${special}`;
  }
  const houseDist = [...combined.matchAll(/\b([a-z]{2})-?(\d{1,2})\b/g)];
  const houseState = [...combined.matchAll(/\b(house|rep)\b.*?\b([a-z]{2})\b|\b([a-z]{2})\b\s*house\b|house\s*\b([a-z]{2})\b/gi)].find(() => true);
...
function classifyOutcomeSemantic(market) {
  const ref = String(market?.ref || '').toLowerCase();
  const title = String(market?.title || '').toLowerCase();
  const text = `${ref} ${title}`;

  if (/#[ ]*(yes|no)$/i.test(String(market?.ref || ''))) return 'binary_yesno';
  if (/\b(democrat|democratic|republican|gop|party)\b/.test(text)) return 'party';
  if (ref.includes('#')) return 'candidate';
  return 'unknown';
}

function classifyElectionPhase(market) {
  const ref = String(market?.ref || '').toLowerCase();
  const title = String(market?.title || '').toLowerCase();
  const text = `${ref} ${title}`;

  if (/\b(primary|nominee|nomination|first round|round 1)\b/.test(text)) return 'primary_or_nominee';
  if (/\b(runoff|second round|round 2)\b/.test(text)) return 'runoff';
  if (/\b(general|winner|wins|win the|election winner|governor race winner)\b/.test(text)) return 'general_or_winner';
  return 'unknown';
}

function semanticGuardResult(blockKey, a, b) {
  const key = String(blockKey || '').toLowerCase();
  if (!key.startsWith('governor_') && !key.startsWith('pres_') && !key.startsWith('nominee')) {
    return { ok: true };
  }

  const typeA = classifyOutcomeSemantic(a);
  const typeB = classifyOutcomeSemantic(b);
  const phaseA = classifyElectionPhase(a);
  const phaseB = classifyElectionPhase(b);

  if ((typeA === 'party' && typeB === 'binary_yesno') || (typeB === 'party' && typeA === 'binary_yesno')) {
    return { ok: false, reason: 'semantic_guard_party_vs_binary_yesno', details: { typeA, typeB, phaseA, phaseB } };
  }

  if ((phaseA === 'primary_or_nominee' && phaseB === 'general_or_winner') || (phaseB === 'primary_or_nominee' && phaseA === 'general_or_winner')) {
    return { ok: false, reason: 'semantic_guard_phase_primary_vs_general', details: { typeA, typeB, phaseA, phaseB } };
  }

  if ((phaseA === 'runoff' && phaseB === 'general_or_winner') || (phaseB === 'runoff' && phaseA === 'general_or_winner')) {
    return { ok: false, reason: 'semantic_guard_phase_runoff_vs_general', details: { typeA, typeB, phaseA, phaseB } };
  }

  return { ok: true, details: { typeA, typeB, phaseA, phaseB } };
}
...
export async function runProposalEngine(opts = {}) {
  ...
  const marketFilterRaw = typeof opts.marketFilter === 'string' ? opts.marketFilter.trim() : '';
  const marketFilter = marketFilterRaw ? marketFilterRaw.toLowerCase() : null;
  const explain = opts.explain === true;
  const isPolitics = category === 'politics';
  const topicTokens = isPolitics ? POLITICS_TOPIC_TOKENS : new Set();
  ...
  report.filtered_semantic_guard = 0;
  ...
  try {
    ...
    // E1.3 guard: exclude expired markets from the candidate pool entirely.
    // Prevents already-settled/closed markets from generating garbage proposals.
    ...
    let kalshiAll = kalshiRows.rows || [];
    let polyAll = polyRows.rows || [];

    if (marketFilter) {
      const keep = (r) => {
        const hay = `${r.provider_market_ref || ''} ${r.title || ''} ${r.event_ref || ''}`.toLowerCase();
        return hay.includes(marketFilter);
      };
      kalshiAll = kalshiAll.filter(keep);
      polyAll = polyAll.filter(keep);
      console.log(
        'pmci:propose:politics market_filter=%s kalshi=%d poly=%d',
        marketFilterRaw,
        kalshiAll.length,
        polyAll.length,
      );
    ...
    function addKalshi(block, m, list) {
      const parsed = parseKalshiTitle(m.provider_market_ref, m.title, topicTokens);
    ...
    function addPoly(block, m, list) {
      ...
      const polyTopicTokens = (parsed.slugTokens || []).filter((t) => topicTokens.has(t));
      const topicKey = extractTopicKey({ title: m.title, provider_market_ref: m.provider_market_ref });
      const topicSignature = extractTopicSignature({ title: m.title, provider_market_ref: m.provider_market_ref });
    ...
    async function considerPair(k, p, blockKey, topicStats, isKalshiSource) {
      ...
      const blockLower = String(blockKey || '').toLowerCase();
      const isGovernorBlock = blockLower.startsWith('governor_');
      const kBlob = `${k.ref || ''} ${k.title || ''}`.toLowerCase();
      const pBlob = `${p.ref || ''} ${p.title || ''}`.toLowerCase();
      const partyStructuredPair =
        (kBlob.includes('govparty') || /\b(democrat|republican|party)\b/.test(kBlob)) &&
        (pBlob.includes('governor') && /\b(democrat|republican|party|winner)\b/.test(pBlob));

      if (isGovernorBlock && partyStructuredPair) {
        // Governor party-vs-party markets are structurally valid but under-scored by generic title similarity.
        proxy_confidence = Math.min(0.97, proxy_confidence + 0.35);
      }
      const MIN_CONFIDENCE_AFTER_ENTITY =
        String(blockKey || '').startsWith('governor_') ? 0.25 :
        String(blockKey || '').startsWith('pres_') || String(blockKey || '').startsWith('nominee') ? 0.40 :
        0.5;
      ...
      const semanticGuard = semanticGuardResult(blockKey, k, p);
      if (!semanticGuard.ok) {
        report.filtered_semantic_guard += 1;
        report.skipped_low_confidence += 1;
        return { wroteEquiv: false, wroteProxy: false };
      }
      ...
      const block = String(blockKey || '').toLowerCase();
      const isGovBlock = block.startsWith('governor_');
      const isPresBlock = block.startsWith('pres_') || block.startsWith('nominee');

      const EQUIV_PENDING_MIN = (isGovBlock || isPresBlock) ? 0.78 : 0.92;
      const PROXY_PENDING_MIN = isGovBlock ? 0.60 : (isPresBlock ? 0.70 : 0.86);

      // E1.3 guard: dedup — skip if the Kalshi market already has an active cross-platform link.
      // Prevents duplicate proposals for markets that are already linked (e.g. Trump nominee).
      if (!dryRun) {
        const _dedupCheck = await client.query(
          `SELECT 1 FROM pmci.market_links WHERE provider_market_id = $1 AND status = 'active' LIMIT 1`,
          [idA],
        );
        if (_dedupCheck.rows.length > 0) {
          report.skipped_already_linked = (report.skipped_already_linked || 0) + 1;
          return { wroteEquiv: false, wroteProxy: false };
        }
      }
      ...
      if (equivalent_confidence >= EQUIV_PENDING_MIN && equivalent_confidence < 0.985) {
        if (report.proposals_written_equivalent >= PMCI_MAX_PROPOSALS_EQUIV) {
          report.caps_hit_equiv = true;
          return { wroteEquiv: false, wroteProxy: false };
        }
        // E1.3 guard: title-similarity floor — reject equivalent proposals where there is
        // insufficient textual overlap. Low title_sim + low slug_sim = entity token matched
        // unrelated events (e.g. "Japan snap election" vs "Trump visits Japan").
        if (reasons.title_similarity < 0.30 && (reasons.slug_similarity ?? 0) < 0.20) {
          report.skipped_low_confidence = (report.skipped_low_confidence || 0) + 1;
          return { wroteEquiv: false, wroteProxy: false };
        }
        if (!dryRun) {
          try {
      ...
      if (proxyEntityGate && proxy_confidence >= PROXY_PENDING_MIN && proxy_confidence < 0.98 && (entityMatch || sharedTopics)) {
